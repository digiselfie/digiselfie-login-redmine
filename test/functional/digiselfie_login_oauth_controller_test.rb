require File.expand_path('../../digiselfie_login_test_helper', __FILE__)

class DigiselfieLoginOauthControllerTest < ActionController::TestCase
  include Helpers::DigiselfieLoginMailHelper
  def setup
    @default_user_credentials = { :firstname => 'Cool',
                                  :lastname => 'User',
                                  :mail => 'user@somedomain.com'}
    @default_response_body = {:FirstName => 'Cool',
                              :LastName => 'User',
                              :Address => 'user@somedomain.com'}
    User.current = nil
    Setting.openid = '1'
  ...
  def test_oauth_digiselfie_with_enabled_oauth_authentification
    Setting.plugin_digiselfie_login['oauth_authentification'] = nil
    get :oauth_digiselfie
    assert_response 404
  end

  def test_oauth_digiselfie_callback_for_existing_non_active_user
    Setting.self_registration = '2'
    user = new_user :status => User::STATUS_REGISTERED
    assert user.save
    set_response_body_stub
    get :oauth_digiselfie_callback
    assert_redirected_to signin_path
  end

  def test_oauth_digiselfie_callback_for_existing_active_user
    user = new_user
    user.activate
    assert user.save
    set_response_body_stub
  ...
  def test_oauth_digiselfie_callback_for_new_user_with_valid_credentials_and_sefregistration_enabled
  ...
  def test_oauth_digiselfie_callback_for_new_user_with_valid_credentials_and_sefregistration_disabled
    Setting.self_registration = '2'
    set_response_body_stub
    get :oauth_digiselfie_callback
    assert_redirected_to signin_path
  ...
  def test_oauth_digiselfie_callback_with_new_user_with_invalid_oauth_provider
    Setting.self_registration = '3'
  ...
  def test_oauth_digiselfie_callback_with_new_user_created_with_email_activation_should_have_a_token
  ...
  def test_oauth_digiselfie_callback_with_new_user_created_with_manual_activation
    Setting.self_registration = '2'
    set_response_body_stub
    get :oauth_digiselfie_callback
    assert_redirected_to :signin
